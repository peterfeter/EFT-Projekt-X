...
namespace eft_dma_radar.UI.Radar
{
    public sealed partial class MainForm : Form
    {
        ...
        {
            if (ColorTranslator.FromHtml(textBox_VischeckVisColor.Text) is Color selectedColor)
            {
                textBox_VischeckVisColor.BackColor = selectedColor;
                textBox_VischeckVisColor.ForeColor = GetContrastingColor(selectedColor);
            }
        }

        private void textBox_VischeckInvisColor_TextChanged(object sender, EventArgs e)
        {
            if (ColorTranslator.FromHtml(textBox_VischeckInvisColor.Text) is Color selectedColor)
            {
                textBox_VischeckInvisColor.BackColor = selectedColor;
                textBox_VischeckInvisColor.ForeColor = GetContrastingColor(selectedColor);
            }
        }

        // Function to get black or white based on contrast
        private Color GetContrastingColor(Color color)
        {
            double luminance = (0.299 * color.R + 0.587 * color.G + 0.114 * color.B) / 255;
            return luminance > 0.5 ? Color.Black : Color.White; // Use white text on dark colors, black text on light colors
        }
